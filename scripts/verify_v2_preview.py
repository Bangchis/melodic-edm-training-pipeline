#!/usr/bin/env python3
"""Clean-redownload the best-val preview and prove one immutable inference."""
...
def main() -> int:
    ...
    clean_root = Path(tempfile.mkdtemp(prefix="melodic-edm-v2-preview.", dir="/workspace"))
    ...
    downloaded_adapter = downloaded / "best-val" / "adapter_model.safetensors"
    packaged_hash = sha256(packaged)
    downloaded_hash = sha256(downloaded_adapter)
    errors = []
    if packaged_hash != downloaded_hash:
        errors.append("downloaded_best_val_adapter_hash_mismatch")
    if inference.get("status") != "pass":
        errors.append("preview_clean_inference_failed")
    probe = inference.get("probe", {})
    if probe.get("sample_rate") != 48000 or probe.get("channels") != 2:
        errors.append("preview_clean_inference_format_invalid")
    report = {
        ...
        "packaged_adapter_sha256": packaged_hash,
        "downloaded_adapter_sha256": downloaded_hash,
        "adapter_hash_match": packaged_hash == downloaded_hash,
        "inference": inference,
        "verified_at": datetime.now(timezone.utc).isoformat(),
        "errors": errors,
    }
    output = root / "outputs" / "v2" / "checkpoint-evaluation" / "preview_clean_verification_report.json"
