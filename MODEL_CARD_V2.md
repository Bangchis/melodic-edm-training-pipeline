...
- Three fused prompt embeddings per record (canonical, composition and production); each starts with the exact row-bound artist/track style reference and independently combines that song's old prompt properties with new MOSS audio evidence.
...
The annotation listener runs under prompt revision `audio-blind-v2.2`, which withholds identity, MIR and prior claims for an independent waveform reading. Named sound sources from both the old per-song annotation and this new reading then pass two independent full-track checks plus an intro/middle/late montage. Claim verifier `multi-view-audio-claims-v2.7` converts only low-confidence binary verdicts to `uncertain` with explicit provenance; malformed or contradictory evidence still fails. Compiler revision `openrouter-per-track-salient-audio-fusion-v3.2` uses `google/gemini-3.1-flash-lite` through OpenRouter as a text-only editor to fuse the old prompt properties of that exact song with the independently heard facts under separately hashed lineage. Exact instrument names are preserved when supported by consensus; absent claims are removed and unresolved timbres are explicitly qualified rather than silently asserted. Only after audio validation, all three trainable views receive the deterministic prefix `Instrumental music in the characteristic style of {artist}, drawing on the musical character of the reference track "{title}".`. A final MOSS listening audit removes this identity prefix before checking caption fidelity against audio, and its evidence is included in the completion audit.
