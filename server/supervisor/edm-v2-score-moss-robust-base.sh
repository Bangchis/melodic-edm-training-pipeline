...
"$project/.venvs/moss-music/bin/python" -u scripts/score_v2_checkpoints_moss.py \
  ...
  --evaluation-dir outputs/v2/robust-base-evaluation
exec python3 -u scripts/summarize_v2_seed_robustness.py \
  --project-root "$project" \
  --report outputs/v2/robust-base-evaluation/listening_scores.json \
  --output outputs/v2/robust-base-evaluation/seed_robustness.json \
  --expected-seeds 5 \
  --minimum-score 3 \
