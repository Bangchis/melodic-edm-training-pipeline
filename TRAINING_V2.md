...
Every record retains one master annotation and three trainable views in this fixed order. The word ranges below apply to the audible body before the row-bound style prefix is prepended:
...
Artist names and source titles are withheld from the waveform listener and from the audio-only caption body while audible facts are being validated. After that validation, every one of the three trainable captions receives the exact deterministic prefix `Instrumental music in the characteristic style of {artist}, drawing on the musical character of the reference track "{title}".` bound to that manifest row. A missing or substituted artist/title fails the dataset audit. BPM, key and time signature remain structured fields. Song form remains in the instrumental lyrics sidecar.

MOSS prompt revision `audio-blind-v2.2` first receives no title, artist, filename, MIR or prior annotation. This prevents plausible catalog context from anchoring the listener on instruments it has not actually heard. Before tensors are accepted, every named sound-source claim from either the old annotation or the independent analysis is checked by two differently worded full-track passes and an intro/middle/late montage. Claim verifier revision `multi-view-audio-claims-v2.7` conservatively normalizes a binary verdict below 0.5 confidence to `uncertain`, records its original verdict and normalization reason, and never upgrades weak evidence to `present`; malformed JSON, missing claims and prose/verdict contradictions still fail and retry. Existing clean v2.5/v2.6 multi-view evidence is revalidated and migrated without listening to the audio again. An exact name is retained when at least two views support it without a strong full-track contradiction; an absent claim is removed. Conflicting evidence keeps the vocabulary token with an explicit `-like` qualifier, such as `pipa-like plucked lead`, rather than either asserting a physical pipa as fact or collapsing the label to a generic `plucked-string-like` phrase.

Compiler revision `openrouter-per-track-salient-audio-fusion-v3.2` then sends two separately hashed text packets for the same `sample_id` to `google/gemini-3.1-flash-lite` through OpenRouter: the old per-track prompt/annotation and the independent waveform analysis. No audio bytes are sent in this step. It preserves distinctive old genre, mood, melody, arrangement and production properties when supported or not contradicted by the audio, while multi-view decisions constrain exact sound-source claims. These decisions are safety constraints, not a keyword checklist: a present claim may be omitted when it is not salient, broad electronic timbre words remain usable, and no global instrument vocabulary is forced across songs. Its deterministic gate rejects newly introduced unverified specific instrument names, embedded BPM, time signature, exact key, quality hype, static-loop cues and generic `standard/classic EDM structure` boilerplate before a caption can enter tensors. Uncertain instruments may be omitted; they are never forced into all captions. The row-bound artist/track style prefix is attached only after these checks, so it cannot influence the audio listener's factual decisions. A final MOSS listening audit strips that prefix and checks only the compiled audible body against the audio.

Each record has three training conditions: canonical, composition and production. Every view begins with the same exact artist/track style reference for that row, then independently combines useful properties from that song's old prompt with the independent new MOSS audio evidence; composition emphasizes melody and arrangement, while production emphasizes instrumentation and sound design. Preprocessing stores one audio latent and three prompt embeddings per record. Training chooses one of the three uniformly on every load with CFG dropout `0.15`; validation always uses canonical index 0 with CFG dropout `0.0`.
...
The generalization gate is separate from the single fixed checkpoint sample. It generates Chinese melodic EDM, gaming progressive house and cinematic glitch-hop at five deterministic seeds each, using long-form section conditioning and artist/track references selected only from the validation split (`China-Future`, `Mannenzakura`, and `Time Lapse`). This tests the new conditioning syntax without using a title/audio pair that the LoRA trained on. At least four of five outputs per style must score `>=3/5` in all four dimensions with no distortion, collapse, static loop or intelligible vocals. Pristine XL-Base receives the identical 15 prompt/seed pairs, and the report records paired Base-only, LoRA-only, both-pass and both-fail outcomes. The separate fixed three-prompt check remains identity-free, so both named-reference and ordinary descriptive prompting are covered. MOSS scores are a repeatable triage signal, not proof that a person will like the music; the release remains subject to human listening.
